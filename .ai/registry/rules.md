...
enforcement map below, which is the point of that table.
...
| RULE-18 | Tracker targets resolve against `tracker.yaml` by ID only. Empty `allowed_list_ids` blocks every call. | 1 | — |
...
| RULE-03 | Review check R1, plus `scripts/check-allowed-paths.mjs` in CI. **The hook is unwired** — see ADR-004. **Both readers exempt the same set** — the ticket folder and the three ship-owned paths (ADR-023 for CI, ADR-041 for R1). **R1 judges uncommitted paths with `node scripts/check-carry.mjs <ID>` instead** — ADR-043, one implementation shared with the runner preflight rather than a prose exempt set. *This row read "R1 has no exemptions" until ADR-041; under that wording CI and R1 read one diff to opposite verdicts, and R1 failed every ticket for the `backlog.md` write that `/triage` and `/advance` make by construction* |

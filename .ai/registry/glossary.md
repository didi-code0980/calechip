...
| **Absence count** | *số người vắng* — the derived number of people away on a given date, over that date's `pending` and `approved` entries: 1 per `full`, 0.5 per `am` or `pm`, PTO and WFH alike. **Rejected entries are excluded.** | A headcount of people. It is a decimal, it is derived and never stored, and it has exactly one definition anywhere in the system (INV-04). |

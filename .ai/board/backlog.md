...
|---|--------|-------|-------|------------|
| 1 | TEA-04 | Remove a member, and promote a member to admin | BACKLOG | TEA-01 |
...
| 16 | TEA-05 | Sign in, sign out, and the member-less landing state | BACKLOG | TEA-01 |
...
**The rows above were renumbered 1–16 by `orchestrator` on 2026-09-01**, when TEA-03 left this
table for `## READY`. `product`'s paragraph is left as written; read its *17* and *sixteen rows* as
the positions at the time it was written. TEA-05 is now row 16 and its position is still nobody's
assertion but a human's to make.

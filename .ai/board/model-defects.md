...
## TODO(project): decide whether this file survives

It overlaps `.ai/board/model-debt.md`, which carries the same kind of entry in a table. The origin
project created both on the same day, numbered them independently, and never merged them — so the
same defect existed twice under two IDs and only one copy was ever updated.

Two registers is worse than either one. Decide now, while both are empty: keep this one for entries
that need a narrative and keep `model-debt.md` for the table, or delete this file. Do not leave the
question open, because the cost only appears once there are entries to reconcile.
